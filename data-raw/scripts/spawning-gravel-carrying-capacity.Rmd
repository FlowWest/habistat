...
comid_with_rm |>
  ...
  ggplot() + 
  geom_sf(data=filter(habistat::flowline_geom_proj, comid %in% habistat::cv_mainstems$comid), color = "#dddddd") +
  geom_sf(aes(color = rm_start), linewidth=1) +
  scale_color_viridis_c(name = "River Mile\n(at DS end of ComID)") +
  ggtitle("Spawning Validation Reaches") 

ggsave(here::here("vignettes", "figures", "spawning-extents-validation-reaches.png"), width=6.5, height=4, dpi=300)
```
...
Apply to an `sf` point dataset via e.g. `data |> mutate(rm_pt = river_miles(geometry, "Feather River"))` where `geometry` is the name of the sf geometry column. Geometry needs to be in the project CRS.
...
**Investigate RM and locations:** Min and max RM seem inconsistent by reach. Lat/lon on some reaches (from datasets with reach name coded with "R") are incorrectly integers. <mark>In lack of usable lat/lon, need to locate river mile definitions in order to correctly compare against habistat estimates.</mark>
...
River miles defined in [USFWS (2018), *Summary of Actions to Jumpstart the Reintroduction of Sacramento River Winter-run Chinook Salmon, to Battle Creek, 2017 – 2018*](https://www.noaa.gov/sites/default/files/legacy/document/2020/Oct/07354626189.pdf), Figure 3:
...
Using manual RM definition on the Battle Creek mainstem only for now, range 0 through 17 (~16.80). Need to confirm if some of these redds were surveyed on the forks.
...
mill_deer_survey_reaches <-
  read_sf(here::here("data-raw", "source", "spawning_data", "mill", "mill_deer_survey_reaches.kml")) |> 
  janitor::clean_names() |> 
  st_zm() %>%
  bind_rows(
    st_as_sf(
      tribble(
        ...
        "Moak Cove", 40.036719,-121.800089,
        "Hwy 36", 40.362721, -121.507514,
        "Power Lines", 40.094243, -121.898560),
      coords = c("longitude", "latitude"),
      crs = st_crs(.))) |>
  st_transform(habistat::const_proj_crs()) |>
  st_join(habistat::cv_watersheds |> select(watershed_level_3, geometry))

mill_survey_reach_endpoints <- 
  mill_deer_survey_reaches |>
  filter(watershed_level_3 == "Mill Creek") |>
  mutate(rm_pt = apply_rm_idx(geometry, mill_rm_idx)) 
# TODO: There is a sequencing issue where HIG erroneously appears US of Little HIG 

mill_survey_reach_endpoints |>
  ggplot(aes(color = rm_pt)) + 
  geom_sf() + 
  geom_sf_text(aes(label = name))

mill_survey_reach_endpoints_lookup <-
  mill_survey_reach_endpoints |>
  st_drop_geometry() |>
  select(name, rm_pt) |>
  deframe()

mill_survey_reach_endpoints_lookup[["Headwaters"]] = max(mill_rm_idx$rm_pt)

mill_reach_endpoint_xw <- 
  read_csv(here::here("data-raw", "source", "spawning_data", "mill", "mill_reach_endpoint_xw.csv")) |>
  mutate(us_rm = mill_survey_reach_endpoints_lookup[us_name],
         ds_rm = mill_survey_reach_endpoints_lookup[ds_name])
...
mill_redd <- 
  read_csv(here::here("data-raw", "source", "spawning_data", "mill", "mill_redd_survey.csv")) |>
  left_join(mill_reach_endpoint_xw, by=join_by(location)) |>
  rename(survey_year = year,
         n_redds = redd_count) |>
  mutate(time_range = "All Time Periods",
         n_redds = coalesce(n_redds, 0))

mill_redds_by_rm <-
  mill_redd |>
  st_drop_geometry() |>
  mutate(rm_start = as.integer(floor(ds_rm))) |>
  group_by(rm_start, survey_year, time_range) |>
  summarize(n_redds = sum(n_redds), .groups="drop") |>
  group_by(survey_year, time_range) |>
  complete(rm_start = seq(min(rm_start), max(rm_start))) |>
...
mill_redds_by_rm |> 
  group_by(rm_start, time_range) |>
  summarize(n_redds = max(n_redds), .groups = "drop") %>%
  bind_rows(expand_grid(rm_start = c(min(.$rm_start) - 1, max(.$rm_start) + 1), 
                        n_redds = 0, 
                        time_range = unique(.$time_range))) |>
  ggplot() +
...
mill_redds_by_rm_no_bin <-
  mill_redd |>
  mutate(rm_start = ds_rm,
         rm_length = us_rm - ds_rm) |>
  st_drop_geometry() |>
  group_by(survey_year, time_range, rm_start, rm_length) |>
  mutate(n_redds = coalesce(n_redds, 0),
         n_redds_per_rm = n_redds / rm_length) |>
  ungroup()

mill_redds_by_rm_no_bin |>
  ggplot() +
  geom_step(aes(x = rm_start, y = n_redds_per_rm, color = time_range)) +
  facet_wrap(~survey_year) +
  ggtitle('Redd Counts for Mill Creek by river mile')

mill_redds_by_rm_no_bin |> 
  group_by(rm_start, time_range) |>
  summarize(n_redds_per_rm = max(n_redds_per_rm), .groups = "drop") |>
  ggplot() +
  geom_step(aes(x = rm_start, y = n_redds_per_rm, color = time_range)) +
  ggtitle('Redd Counts for Mill Creek by river mile (upper envelope)')
```
...
See Mill Creek dataset above for survey endpoint locations
...
```{r combined_plot, fig.width=8, fig.height=10.5, dpi=300}
...
```{r simple_plot, fig.width=6.5, fig.height=6.5, dpi=300}
# redd_max <-
#   redd_data |>
...
```{r simple_plot_v2, fig.width=6.5, fig.height=6.5, dpi=300}
redd_data <- list("Lower Yuba River" = yuba_redds_by_rm,
                  "Feather River" = feather_redds_by_rm,
                  "Clear Creek" = clear_redds_by_rm,
                  "Mill Creek" = mill_redds_by_rm_no_bin,
                  "Battle Creek" = battle_redds_by_rm)

for (x in names(redd_data)) {
  
  min_rm <- floor(min(comid_with_rm$rm_start[which(comid_with_rm$river_name==x)])) - 1
  max_rm <- ceiling(max(comid_with_rm$rm_end[which(comid_with_rm$river_name==x)])) + 1
  
  dta <- spawning_context_comid_rm |> filter(river_name == x)
  
  plt_context <- 
    spawning_context_comid_rm |>
    filter(river_name == x) |>
    ggplot(aes(xmin = rm_start, xmax = rm_end, ymin = -1, ymax = 1)) + 
    geom_rect(data = filter(dta, spawning_geographic_context), aes(fill = "Spawning Geographic Scope")) +
    geom_rect(data = filter(dta, spawning_filter_all), aes(fill = "Spawning Gravel Likelihood")) +
    theme(panel.grid.major.y = element_blank(), 
          panel.grid.minor.y = element_blank(), 
          axis.text.y = element_blank(),
          legend.position = "top") +
    ...
    scale_fill_manual(name = "",
                      values = c("Spawning Geographic Scope" = "gray",
                                 "Spawning Gravel Likelihood" = "black"))
  
  plt_preds <- 
    spawning_pred_comid_rm |>
    filter(river_name == x) |>
    filter(flow_idx %in% c(1000, 10000)) |>
    ggplot() +
    facet_wrap(~str_glue("{flow_idx} cfs"), ncol=1) +
    geom_rect(aes(xmin = rm_start, xmax = rm_end, ymin = 0, ymax = wua_ac_per_rm), fill = "black", color="white") +
    ylab("Potential Spawning Area (ac) per RM") + xlab("River Mile") + 
    labs(subtitle="Habistat Spawning Area Predictions") + 
    scale_color_viridis_c(direction = -1)
  
  rdta <- redd_data[[x]]
  if (!("n_redds_per_rm" %in% names(rdta))) {
    rdta <- 
      rdta |>
      mutate(n_redds_per_rm = n_redds,
             rm_length = 1)
  } 
  
  plt_redds <- 
    rdta |>
    group_by(rm_start, rm_length, time_range) |>
    summarize(n_redds_per_rm = max(n_redds_per_rm)) |>
    drop_na(time_range) |>
    ggplot() + 
    geom_rect(aes(xmin = rm_start, xmax = rm_start + rm_length, ymin = 0, ymax = n_redds_per_rm), 
              fill = "black", color = "white") +
    facet_wrap(~time_range, ncol=1) +
    xlab("River Mile") + ylab("Number of Redds per RM") + 
    labs(subtitle="Redd Surveys (upper envelope)") + 
    scale_y_continuous(sec.axis = sec_axis(name = "Redd Area (ac) per RM", transform = ~.*94/43560)) +
    theme(panel.grid.minor = element_blank(), legend.position="top",
          strip.text.y = element_text(angle = 0), strip.placement = "outside")
  ...
  plt_combined <- (plt_context / plt_preds / plt_redds) + 
    plot_layout(axes = "collect", heights=c(1,3,3)) +
    plot_annotation(title = x) &
    coord_cartesian(xlim = c(min_rm, max_rm)) &
    scale_x_continuous(breaks=scales::breaks_extended(n = 10)) 
  
  print(plt_combined)
  
  ggsave(here::here("vignettes", "figures", 
                    str_glue("spawning-extents-val-{janitor::make_clean_names(x, sep_out='-')}.png")), 
