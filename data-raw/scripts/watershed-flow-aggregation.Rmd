...
wua_hydraulic |>
  filter(habitat == "rearing") |>
  filter(dataset == "Mokelumne River") |>
  ggplot() +
  geom_line(aes(x = flow_cfs, y = wua_per_lf, group = comid)) +
  scale_x_log10(limits = c(50, 15000)) +
  scale_y_continuous(limits = c(0, 250))

wua_predicted |>
  filter(habitat == "rearing") |>
  filter(comid %in% filter(wua_hydraulic, dataset == "Mokelumne River")$comid) |>
  ggplot() +
  geom_line(aes(x = flow_cfs, y = wua_per_lf_pred, group = comid))  +
  scale_x_log10(limits = c(50, 15000)) +
  scale_y_continuous(limits = c(0, 250))
```
...
```{r}
habistat::flowline_geom_proj |>
  filter(comid %in% filter(cv_mainstems, river_group == "Butte Creek")$comid) |>
  inner_join(habistat::flowline_attr |> select(comid, starts_with("da_area"))) |>
  View()# |>
  pivot_longer(cols = starts_with("da_area")) |>
  ggplot() + 
  facet_wrap(~name) +
