...
dsm_habitat_combined <- mainstems |> 
  ...
  transmute(river_group = watershed, flow_cfs, 
  ...
  ungroup() |>
  mutate(regional_approx = river_group %in% regional_approx_groups)

dsm_habitat_wua_per_lf <- dsm_habitat_combined |>
  select(river_group, flow_cfs, instream_wua_per_lf, floodplain_wua_per_lf, regional_approx) |>
...
dsm_habitat_suitable_ac <- dsm_habitat_combined |>
  select(river_group, flow_cfs, instream_suitable_ac, floodplain_suitable_ac, regional_approx) |>
...
wua_predicted_cv_mainstems_grouped |>
  ...
  filter(river_group %in% dsm_habitat_wua_per_lf$river_group) |>
  ggplot(aes(x = flow_cfs)) + 
  facet_wrap(~river_group, scales="free") + 
  ...
  geom_line(data=dsm_habitat_suitable_ac |> 
            ...
            aes(x = flow_cfs, y = value, color = paste("vs", name), linetype = regional_approx)) +
  scale_x_log10() +
  theme(legend.position = "top", panel.grid.minor = element_blank()) + 
...
Removing the ones that use the "[regional approximation](http://cvpia-habitat-docs-markdown.s3-website-us-west-2.amazonaws.com/watershed/Regional_Approximation.html)"

```{r pred-watersheds-dsmhabitat-comparison-filtered, fig.width=8.5, fig.height=8.5, dpi=300}
wua_predicted_cv_mainstems_grouped |>
  filter(habitat == "rearing") |>
  filter(river_group %in% dsm_habitat_wua_per_lf$river_group) |>
  filter(!(river_group %in% regional_approx_groups)) |>
  ggplot(aes(x = flow_cfs)) + 
  facet_wrap(~river_group, scales="free") + 
  geom_ribbon(aes(ymin = wua_acres_pred_SD, ymax = wua_acres_pred_SN, fill = "habistat rearing"), alpha=0.33) +
  geom_line(aes(y = wua_acres_pred, color = "habistat rearing")) +
