...
scale_fp_flow_area <- function(ws) {
  ...
  # TODO- modify so it works with any watershed
  # TODO - start by generating all (Deer, Tuolumne, Cottonwood) and then we can filter out
  # geographically later 
  proxy_data <- switch(watershed_metadata$scaling_watershed, # proxy_watershed
...
watersheds_fp_scale <- DSMhabitat::floodplain_modeling_metadata |> filter(!is.na(dec_jun_mean_flow_scaling)) |> 
  pull(watershed)
