...
```{r}
# Version 2
spawning_hsc_vel_v2 <- spawning_hsc |> 
  filter(suitability_metric == "Velocity") |> 
  filter(units_si < 30,
         species == "Chinook") |>   
  group_by(race) %>%
  complete(river_citation, units_si = unique(.$units_si)) |>
  group_by(race, river_citation) |>
  mutate(suitability_index = zoo::na.approx(suitability_index, x = units_si, na.rm=F)) |>
  group_by(race, units_si) |>
  summarise(max_suit_index = max(suitability_index, na.rm=T)) 
...
spawning_hsc_vel_v2 |> 
  ggplot() + 
  geom_line(aes(x = units_si / 0.3048, y = max_suit_index, color = race), size = 1) + 
  geom_line(data = spawning_hsc_vel_v2 |> 
              group_by(units_si) |> 
              summarize(max_suit_index = max(max_suit_index, na.rm=T)),
            aes(x = units_si / 0.3048, y = max_suit_index, linetype = "Max"), size = 1) +
  xlab('velocity (feet/second)')  + 
  ylab('suitability index')  +
  scale_linetype_manual(name = "Summary", values = "dashed")
```
...
```{r, echo = FALSE}
# new version without rounding error
spawning_vel_hsi <-
  spawning_hsc_vel_v2 |>
  rename(run = race) |>
  mutate(run = janitor::make_clean_names(run, allow_dupes=T)) |>
  group_by(run) |>
  mutate(units_us = units_si / 0.3048) |>
  reframe(vel_ft_s = seq(0,10,0.1),
          hsi = approx(x = units_us, y = max_suit_index, xout = vel_ft_s)$y) |>
  mutate(hsi = if_else(is.na(hsi) | is.nan(hsi) | hsi < 0, 0, hsi)) %>%
  bind_rows(. |> 
              group_by(vel_ft_s) |> 
              summarize(hsi = max(hsi, na.rm=T), 
                        run = "max") |>
              ungroup()) |>
  mutate(vel_m_s = vel_ft_s * 0.3048) |>
  relocate(vel_m_s, .before = vel_ft_s)
...
suitability_hsi_vel_spawning |>
  ...
  ggplot() +
  geom_line(aes(x = vel_ft_s, y = hsi, color = name), size = 1) +
  geom_line(data = suitability_hsi_vel_spawning$max,
            aes(x = vel_ft_s, y = hsi, linetype = "Max"), size = 1) + 
  xlab('depth (feet)') + 
  ylab('suitability index') +
  scale_linetype_manual(name = "Summary", values = "dashed") +
